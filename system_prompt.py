...
system_prompt.py
-----------------
System prompt dedicado do GoodWe EV ChargeOps Chatbot.

Movido para arquivo próprio (fora do README) conforme melhoria C1 do
feedback da Sprint 1. Estruturado em seções padronizadas:

PAPEL, ESCOPO, CONTEXTO_TECNICO, REGRAS_DE_COMPORTAMENTO,
FORMATO_DE_SAIDA, ESCALADA_HUMANA, LIMITES_E_SEGURANCA, FERRAMENTAS.

Também define EXEMPLOS_FEW_SHOT: pares de pergunta/resposta (um por
persona) usados como mensagens few-shot no início do histórico, para
calibrar o tom e o formato de resposta do modelo (diferencial de
few-shot prompting).
"""
...
# FEW-SHOT EXAMPLES
# ---------------------------------------------------------------------
# Pares de pergunta/resposta "ideais", um por persona, usados como
# mensagens few-shot logo após o system prompt no histórico inicial da
# conversa. Calibram tom, nível de detalhe e formato (FORMATO_DE_SAIDA)
# antes mesmo da primeira pergunta real do usuário.
#
# Cada item é (pergunta_usuario, resposta_assistente).
EXEMPLOS_FEW_SHOT = [
    (
        "Olá, sou morador do apartamento 102. Quanto eu já gastei "
        "com carregamento este mês?",
        (

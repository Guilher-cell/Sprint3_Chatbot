...
## Nota de metodologia (importante)

A medição de tokens **exata** deveria usar `tiktoken` (Aula 04), mas o ambiente onde este
pacote de arquivos foi montado não tem acesso de rede ao endpoint de download do vocabulário
BPE da OpenAI (`openaipublic.blob.core.windows.net`), então os números de tokens acima são uma
**estimativa por caracteres/4** (heurística comum, mas aproximada). O comparativo de
caracteres e palavras, por outro lado, é **exato** (calculado diretamente sobre os dois
arquivos `.md`).

Para obter a contagem exata de tokens localmente, execute:

```bash
python evals/measure_tokens.py
```

Esse script usa `tiktoken.get_encoding("cl100k_base")` como proxy de tokenizador (o
`gpt-oss:120b` via Ollama não expõe um tokenizador público equivalente ao `cl100k_base`;
`cl100k_base` é usado aqui apenas como referência comparativa consistente entre v1 e v2, não
como contagem exata de tokens do modelo Ollama real).
